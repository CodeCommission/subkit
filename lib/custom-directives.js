...
function resolveWithDirective(resolve, source, directive, context, info) {
  ...
  for (const arg of directive.arguments) {
    args[arg.name.value] = arg.value.value || GraphQLJSON.parseLiteral(arg.value)
  }
...
function parseSchemaDirectives(directives) {
  ...
  for (const directiveName in directives) {
    ...
    Object.keys(directives[directiveName]).forEach(key => argsList.push(`${key}:"${directives[directiveName][key]}"`))
...
function wrapFieldsWithMiddleware(fields) {
  for (const label in fields) {
    ...
    if (!!field && typeof field == 'object' && !field.applyed) {
      ...
      if (field.type.ofType && field.type.ofType._fields) wrapFieldsWithMiddleware(field.type.ofType._fields)
    }
  }
}
...
function addCustomDirectivesToSchema(schema, customDirectives = []) {
  if (!(schema instanceof GraphQLSchema)) throw new Error('Schema must be instanceof GraphQLSchema')
  ...
  schema._mutationType && wrapFieldsWithMiddleware(schema._mutationType._fields)
  return true
}
